...
极简、易于二次开发的 AI 推理网关，适合团队内部共享 LLM API 资源。

统一管理多个上游提供商的 API Key，对内分发独立配额的网关 Key，按 token 用量自动计费。对外暴露 OpenAI 兼容接口和 Anthropic 兼容接口，接入方零改造。
...
- **多提供商路由** — OpenAI / Anthropic / Gemini / Qwen，按优先级 + 权重分配流量
- **双协议** — OpenAI 格式 (`/v1/chat/completions`) + Anthropic 格式 (`/v1/messages`)，支持 Claude Code 直连
- **按量计费** — Prompt / Completion / Cached Token 三种价格，内置 49 个主流模型价格一键导入
- **Key 隔离** — 每个 Key 独立配额、并发限制、模型白名单、过期时间
- **管理后台** — 渠道管理、Key 管理、模型定价、用量统计、请求日志
