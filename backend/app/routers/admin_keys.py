...
from app.services.auth import get_current_admin, hash_api_key
...
async def create_key(body: APIKeyCreate, _: Admin = Depends(get_current_admin)):
...
async def update_key(key_id: int, body: APIKeyUpdate, _: Admin = Depends(get_current_admin)):
    k = await APIKey.get_or_none(id=key_id)
    if k is None:
        raise HTTPException(status_code=404, detail="Key not found")
...
async def delete_key(key_id: int, _: Admin = Depends(get_current_admin)):
...
async def reset_quota(key_id: int, _: Admin = Depends(get_current_admin)):
